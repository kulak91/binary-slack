...
const adminUsers = process.env?.SLACK_ADMINS.split(' ');


const app = new App({
  token: process.env["SLACK_BOT_TOKEN"],
  signingSecret: process.env["SLACK_SIGNING_SECRET"],
  port: process.env["SLACK_PORT"] || 6666,
...
app.message('info_app', async ({ message, client, say, payload }) => {
  ...
  try {
    ...
    const filePath = process.env.LOGS_PATH;
    const data = await fs.readFile(filePath);
    const fileName = 'strapi-out.log';
    await client.files.upload({ file: data, channels: message.channel, title: date.toLocaleString(), filename: fileName, token: process.env["SLACK_BOT_TOKEN"] });
  } catch (err) {
    console.log(err);
  }
